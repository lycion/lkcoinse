...
if "%BUILD_ARCH%" NEQ "T" (
	...
	echo of the Lkcoinse client in SET_ENV_VARS.bat.
...
if not exist "%MINGW_ROOT%" (
	...
	echo Aborting build of Lkcoinse client...
	pause
	exit /b -1
)
...
if not exist "%MSYS_SH%" (
	echo MSYS does not appear to have installed correctly.  Build of Lkcoinse client
...
if "%BUILD_32_BIT%" NEQ "" (
	...
	set "BUILD_OUTPUT=%LKCOINSE_GIT_ROOT%\build-output\x86"
...
if "%BUILD_64_BIT%" NEQ "" (
	...
	set "BUILD_OUTPUT=%LKCOINSE_GIT_ROOT%\build-output\x64"
...
%MSYS_SH% "%INST_DIR%\make-lkcoinse.sh"
REM Check to see if make-lkcoinse.sh failed (possibly due to missing dependencies)
if %errorlevel% neq 0 (
...
echo Saving lkcoinse executables to %BUILD_OUTPUT%
REM Make sure output directory exists
mkdir "%BUILD_OUTPUT%\"

REM cd to src to copy lkcoinse-tx.exe, lkcoinse-cli.exe, and lkcoinsed.exe
cd "%LKCOINSE_GIT_ROOT%\src"
copy lkcoinse-tx.exe "%BUILD_OUTPUT%\lkcoinse-tx.exe"
copy lkcoinse-cli.exe "%BUILD_OUTPUT%\lkcoinse-cli.exe"
copy lkcoinsed.exe "%BUILD_OUTPUT%\lkcoinsed.exe"
copy lkcoinse-miner.exe "%BUILD_OUTPUT%\lkcoinse-miner.exe"

REM cd to src\qt to copy lkcoinse-qt.exe
cd qt
copy lkcoinse-qt.exe "%BUILD_OUTPUT%\lkcoinse-qt.exe"
