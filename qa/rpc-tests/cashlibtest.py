...
class MyTest (LkcoinseTestFramework):
    ...
    def run_test(self):
        ...
        try:
            ...
            faulted = True
            pass
        assert faulted, "not signing with lkcoinse cash forkid"
...
def Test():
    ...
    if not os.path.exists(os.path.abspath(here + "/../../src/lkcoinsed")):
        ...
        cashlib.init(objpath + os.sep + ".libs" + os.sep + "liblkcoinsecash.so")
    else:
        cashlib.init(os.path.abspath(here + "/../../src/.libs/liblkcoinsecash.so"))
