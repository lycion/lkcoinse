...
def init(liblkcoinsecashfile=None):
    global cashlib
    if liblkcoinsecashfile is None:
        liblkcoinsecashfile = "liblkcoinsecash.so"
        try:
            cashlib = CDLL(liblkcoinsecashfile)
        except OSError:
            import os
            dir_path = os.path.dirname(os.path.realpath(__file__))
            cashlib = CDLL(dir_path + os.sep + liblkcoinsecashfile)
    else:
        cashlib = CDLL(liblkcoinsecashfile)
    if cashlib is None:
        raise Error("Cannot find %s shared library", liblkcoinsecashfile)
...
def signTxInput(tx, inputIdx, inputAmount, prevoutScript, key, sigHashType=SIGHASH_FORKID | SIGHASH_ALL):
    ...
    """
    assert (sigHashType & SIGHASH_FORKID) > 0, "Did you forget to indicate the lkcoinse cash hashing algorithm?"
    if type(tx) == str:
        tx = unhexlify(tx)
    elif type(tx) != bytes:
        tx = tx.serialize()
