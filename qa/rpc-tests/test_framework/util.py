...
#
import pdb
# Add python-lkcoinserpc to module search path:
import os
import sys
import math
import binascii
...
LKCOINSED_PROC_WAIT_TIMEOUT = 60
...
class NoConfigValue:
    """ Use to remove the specific configure parameter and value when writing to lkcoinse.conf"""
    def __init__(self):
        pass
...
def enable_mocktime():
    # Set the mocktime to be after the Lkcoinse Cash fork so
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting LCS values"""
...
def initialize_datadir(dirname, n, lkcoinseConfDict=None, wallet=None, bins=None):
    ...
    if lkcoinseConfDict: defaults.update(lkcoinseConfDict)
...
def wait_for_lkcoinsed_start(process, url, i):
    '''
    Wait for lkcoinsed to start. This means that RPC is accessible and fully initialized.
    Raise an exception if lkcoinsed exits during initialization.
    '''
    while True:
        if process.poll() is not None:
            raise Exception('lkcoinsed exited with status %i during initialization' % process.returncode)
        try:
...
def initialize_chain(test_dir,lkcoinseConfDict=None,wallets=None, bins=None):
    """
    ...
    if (not os.path.isdir(os.path.join("cache","node0"))
        ...
        # Create cache directories, run lkcoinseds:
        for i in range(4):
            for retry in range(4):
                try:
                    datadir=initialize_datadir("cache", i, lkcoinseConfDict, wallets, bins)
                    if bins:
                        args = [ bins[i], "-keypool=1", "-datadir="+datadir ]
                    else:
                        args = [ os.getenv("LKCOINSED", "lkcoinsed"), "-keypool=1", "-datadir="+datadir ]
                    if i > 0:
                        args.append("-connect=127.0.0.1:"+str(p2p_port(0)))
                    lkcoinsed_processes[i] = subprocess.Popen(args)
                    if os.getenv("PYTHON_DEBUG", ""):
                        print("initialize_chain: lkcoinsed started, waiting for RPC to come up")
                    wait_for_lkcoinsed_start(lkcoinsed_processes[i], rpc_url(i), i)
                    if os.getenv("PYTHON_DEBUG", ""):
                        print("initialize_chain: RPC succesfully started")
                    break
                except Exception as exc:
                    logging.error("Error bringing up lkcoinsed #%d (initialize_chain, directory %s), this might be retried. Problem is:", i, test_dir)
                    do_and_ignore_failure(lkcoinsed_processes[i].kill)
                    traceback.print_exc(file=sys.stdout)
                    remap_ports(i)
            else:
                raise Exception("Couldn't start lkcoinsed even with retries on different ports (initialize_chain).")
    ...
    for i in range(4):
        ...
        initialize_datadir(test_dir, i,lkcoinseConfDict,wallets[i] if wallets else None) # Overwrite port/rpcport in lkcoinse.conf

def initialize_chain_clean(test_dir, num_nodes, lkcoinseConfDict=None, wallets=None):
    """
    Create an empty blockchain and num_nodes wallets.
    Useful if a test case wants complete control over initialization.
    """
    for i in range(num_nodes):
        datadir=initialize_datadir(test_dir, i, lkcoinseConfDict, wallets)
...
def start_node(i, dirname, extra_args=None, rpchost=None, timewait=None, binary=None):
    """
    Start a lkcoinsed and return RPC connection to it
    """
    datadir = os.path.join(dirname, "node"+str(i))
    if binary is None:
        binary = os.getenv("LKCOINSED", "lkcoinsed")
    ...
    for retry in range(4):
        try:
            lkcoinsed_processes[i] = subprocess.Popen(args)
            if os.getenv("PYTHON_DEBUG", ""):
                print("start_node: lkcoinsed started, waiting for RPC to come up")
            url = rpc_url(i, rpchost)
            wait_for_lkcoinsed_start(lkcoinsed_processes[i], url, i)
            if os.getenv("PYTHON_DEBUG", ""):
                print("start_node: RPC succesfully started")
            break
        except Exception as exc:
            logging.error("Error bringing up lkcoinsed #%d (start_node, directory %s), this might be retried. Problem is:", i, dirname)
            do_and_ignore_failure(lkcoinsed_processes[i].kill)
            traceback.print_exc(file=sys.stdout)
            remap_ports(i)
            fixup_ports_in_configfile(i)
    else:
        raise Exception("Couldn't start lkcoinsed even with retries on different ports (start_node).")
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None, binary=None,timewait=None):
    """
    Start multiple lkcoinseds, return RPC connections to them
    """
...
def stop_node(node, i):
    ...
    lkcoinsed_processes[i].wait(timeout=LKCOINSED_PROC_WAIT_TIMEOUT)
    del lkcoinsed_processes[i]
...
def wait_lkcoinseds():
    # Wait for all lkcoinseds to cleanly exit
    for lkcoinsed in lkcoinsed_processes.values():
        lkcoinsed.wait(timeout=LKCOINSED_PROC_WAIT_TIMEOUT)
    lkcoinsed_processes.clear()
...
def split_transaction(node, prevouts, toAddrs, txfeePer=DEFAULT_TX_FEE_PER_BYTE,**kwargs):
    ...
    try:  # try finally block to put the decimal precision back to what it was prior to this routine
      decimal.getcontext().prec = 8 + 8 # 8 digits to get to 21million, and each lkcoinse is 100 million satoshis
      amount = Decimal(0)
      iamount = 0
      count = 0
      for tx in prevouts:
        inp.append({"txid":str(tx["txid"]),"vout":tx["vout"]})
        amount += tx["amount"]*Decimal(LCS)
        iamount += int(tx["amount"]*Decimal(LCS))
        count += 1
      ...
      while 1:
          ...
          for a in toAddrs[0:-1]:
            if PerfectFractions:
              outp[str(a)] = str(amtPer/Decimal(LCS))
            else:
              outp[str(a)] = float(amtPer/LCS)

          a = toAddrs[-1]
          amtPer = (amount - ((len(toAddrs)-1)*amtPer)) - txfee
          # print "final amt: ", amtPer
          if PerfectFractions:
              outp[str(a)] = str(amtPer/LCS)
          else:
              outp[str(a)] = float(amtPer/LCS)
