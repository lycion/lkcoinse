...
def lkcoinseAddress2bin(lcsAddress):
    """convert a lkcoinse address to binary data capable of being put in a CScript"""
    # chop the version and checksum out of the bytes of the address
    return decodeBase58(lcsAddress)[1:-4]
...
def createWastefulOutput(lcsAddress):
    """ Warning: Creates outputs that can't be spent by lkcoinsed"""
...
this is junk data. this is junk data. this is junk data. this is junk data. this is junk data."""
    ret = CScript([data, OP_DROP, OP_DUP, OP_HASH160, lkcoinseAddress2bin(lcsAddress), OP_EQUALVERIFY, OP_CHECKSIG])
    return ret


def p2pkh(lcsAddress):
    """ create a pay-to-public-key-hash script"""
    ret = CScript([OP_DUP, OP_HASH160, lkcoinseAddress2bin(lcsAddress), OP_EQUALVERIFY, OP_CHECKSIG])
    return ret


def createrawtransaction(inputs, outputs, outScriptGenerator=p2pkh):
    """
    Create a transaction with the exact input and output syntax as the lkcoinse-cli "createrawtransaction" command.
    If you use the default outScriptGenerator, this function will return a hex string that exactly matches the
    output of lkcoinse-cli createrawtransaction.

    But this function is extended beyond lkcoinse-cli in the following ways:
    ...
    for addr, amount in pairs:
        if callable(addr):
            tx.vout.append(CTxOut(amount * LCS, addr()))
        elif type(addr) is list:
            tx.vout.append(CTxOut(amount * LCS, CScript(addr)))
        elif addr == "data":
            tx.vout.append(CTxOut(0, CScript([OP_RETURN, unhexlify(amount)])))
        else:
            tx.vout.append(CTxOut(amount * LCS, outScriptGenerator(addr)))
    tx.rehash()
