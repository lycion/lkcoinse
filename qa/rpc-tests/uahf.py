...
invalidOpReturn = hexlify(b'Lkcoinse: A Peer-to-Peer Electronic Cash System')

class BUIP055Test (LkcoinseTestFramework):
    def __init__(self, extended=False):
        self.extended = extended
        LkcoinseTestFramework.__init__(self)
    ...
    def testDefaults(self):
        for n in self.nodes:
            ...
            if int(nodeInfo["localservices"],16)&NODE_LKCOINSE_CASH:
                assert(t['mining.forkTime'] == 1501590000)  # Lkcoinse Cash release REQ-2
            else:
    ...
    def testCli(self):
        ...
        # if this is a lkcoinse cash build, we need to do the cash defaults on our old chain node
        if int(nodeInfo["localservices"],16)&NODE_LKCOINSE_CASH:
    ...
    def testNetMagic(self):
        ...
        if int(info["localservices"],16)&NODE_LKCOINSE_CASH:
            try: # Accept BU cash nodes if running LCS node
                buCashNode.cnxns[0].wait_for_buverack()
            except DisconnectedError:
                assert(not "should not have disconnected a lkcoinse cash node")
        else:
            try: # do not accept BU cash nodes if running LCS node
                buCashNode.cnxns[0].wait_for_buverack()
                assert(not "should have disconnected a lkcoinse cash node")
            except DisconnectedError:
...
def Test():
    ...
    t.main(["--tmpdir=/ramdisk/test","--nocleanup","--noshutdown"], lkcoinseConf, None)  # , "--tracerpc"])
