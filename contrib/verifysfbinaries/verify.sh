...
###   This script attempts to download the signature file SHA256SUMS.asc from lkcoinse.org
...
WORKINGDIR="/tmp/lkcoinse"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test/"
BASEDIR="https://lkcoinse.org/bin/"
VERSIONPREFIX="lkcoinse-unlimited-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'lkcoinse-',
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Lkcoinse binary release signing key installed?"
   fi
