#!/bin/bash
# Copyright (c) 2013 The Lkcoinse Core developers
...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Lkcoinse database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Lkcoinse datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Lkcoinse datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Lkcoinse 0.7 datadir."
    ;;
  3)
    echo "Detected Lkcoinse pre-0.8 datadir."
    ;;
  4)
    echo "Detected Lkcoinse 0.8 datadir."
    ;;
esac
