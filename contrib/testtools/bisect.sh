...
# based on https://github.com/ftrader/lkcoinse-misc-contrib/blob/master/bisection-tools/bisect_probabilistic_with_timeout.sh
# relicensed by freetrader under the MIT software license (see below)
#########################################################################
# Copyright (c) 2017 The Lkcoinse Unlimited developers
...
#
# 1. CAUTION!!!! This kills any running lkcoinsed's as part of cleanup!
...
# optional tool for use with "test_lkcoinse" tests
...
# sequentially.
PARALLEL_TEST_TOOL=gtest-parallel-lkcoinse
...
TEST_COMMAND=${3:-"src/test/test_lkcoinse"}
...
# helper function to check if parameter is some form of test_lkcoinse
# this function has been dumbed down for now.
is_test_lkcoinse()
{
    if [ "$1" = "src/test/test_lkcoinse" ]
    then
        return 0
    else
        return 1
...
do
    ...
    # check if the test is src/test/test_lkcoinse
    is_test_lkcoinse "${TEST_COMMAND}"
    if [ $? -eq 0 ]
    then
