Release Notes for Lkcoinse Unlimited Cash Edition 1.4.0.0
=========================================================

Lkcoinse Unlimited Cash Edition version 1.4.0.0 is now available from:
...
https://github.com/lkcoinsecashorg/lkcoinsecash.org/blob/master/spec/uahf-technical-spec.md (Aug 1st '17 Protocol Upgrade, bucash 1.1.0.0)
https://github.com/lkcoinsecashorg/lkcoinsecash.org/blob/master/spec/nov-13-hardfork-spec.md (Nov 13th '17 Protocol Upgrade, bucash 1.1.2.0)
https://github.com/lkcoinsecashorg/lkcoinsecash.org/blob/master/spec/may-2018-hardfork.md (May 15th '18 Protocol Upgrade, bucash 1.3.0.0, 1.3.0.1, 1.4.0.0)
...
- Rework of the Lkcoinse fuzzer command line driver tool
...
- Cashlib: create a shared library to make creating wallets easier (experimental, this library factors useful functionality out of lkcoinsed into a separate shared library that is callable from higher level languages. Currently supports transaction signing, additional functionality TBD)
...
- `ec59b18e9` unittests: Default enable all logging with '--log_lkcoinse console' (Awemany)
...
- `64945e80d` Fix variables shadowing qt/lkcoinsegui.cpp (Andrea Suisani)
...
- `45dce18fc` Fix variable shadowing in test/test_lkcoinse_fuzzy.cpp (Andrea Suisani)
...
- `637417598` Add src/lkcoinse-miner to .gitignore (#1180) (Peter Tschipper)
...
- `f5a748bc6` Update windows rebuild-lkcoinse.bat script to account for lkcoinse-miner.exe (Peter Tschipper)
...
- `f770c3fbf` This is a port of lkcoinse/lkcoinse#9753 and should fix issue #1144 (Russell Yanofsky)
...
- `6bf16426c` travis: Dump available debug.logs and lkcoinse.confs in case of test failure (Awemany)
...
- `463618fd2` lkcoinse_qt.m4: Add missing dollar sign for variable. (Werner Lemberg)
- `e5a2e8cbd` lkcoinse_qt.m4: Add missing braces around variables in autoconf messages. (Werner Lemberg)
- `5b2602e3c` lkcoinse_qt.m4: Use correct M4 quoting characters. (Werner Lemberg)
- `4297df1f5` lkcoinse_qt.m4: Improve QT_VERSION tests. (Werner Lemberg)
...
- `a81c2443e` test_lkcoinse_fuzzy: Further updates (Awemany)
...
- `6b4864046` Changed lkcoinse-miner to work with changes in submitminingsolution function. Fixed help messages. (lonoami)
- `3f94e396e` pass univalue object not stringified, and properly parse the miningsolution return (Andrew Stone)
- `fbc948907` move miner-specific allowed args into lkcoinse-miner.  exchange JSON not string-encoded json, and regularize json dictionary names with other RPC calls.  Add python unit test (Andrew Stone)
...
- `73d6e316b` lkcoinse-miner: Added help message, ability to set blockversion and it now uses its own time (lonoami)
- `07286fd4f` Lkcoinse mining documentation added (lonoami)
...
- `462eaa068` multithread lkcoinse-miner, increment candidate ids rather than use a random number, clean up old mining candidates ONLY if they are old (Andrew Stone)
- `50b9de830` Changed submit candidate to retry sending to lkcoinsed. Before only once. (lonoami)
- `f80bf782f` Fully implemented Lkcoinse Candidate (lonoami)
- `e65858a5d` add lkcoinse-miner example CPU miner.  Document merkle API calls and use them in the new mining block candidate RPC calls (Andrew Stone)
...
- `1a4b31e2f` Rework of the Lkcoinse fuzzer command line driver tool (Awemany)
...
- `638b66ecd` Port XT PR #403: [RPC][Lkcoinse-TX] Add support for sequence number (Tom Harding)
...
We have backported an amount of changes from other projects, namely Lkcoinse Core, Lkcoinse XT and Lkcoinse ABC.
