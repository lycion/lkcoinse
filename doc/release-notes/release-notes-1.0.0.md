Release Notes for Lkcoinse Unlimited v1.0.0
==========================================

Lkcoinse Unlimited version 1.0.0 is now available from:
...
The third official BU client release reflects our opinion that Lkcoinse full-node
...
stalled network effect growth, based on Lkcoinse Unlimited's Emergent Consensus
model to let the ecosystem decide the best values of parameters like the maximum
block size.

Lkcoinse Unlimited open-source version 1.0.0 contains a large number of changes,
...
imported from the open source work of the Lkcoinse Core developers who deserve
...
In co-operation with Lkcoinse XT a potential attack on SG has been identified. A
...
Lkcoinse Unlimited recommends miners and commercial node users set their
...
When a Lkcoinse node validates transactions, it verifies signatures. These
...
a use-cases for allowing transactions >1MB. The command for the lkcoinse.conf is
...
Further information:
- [Github Repository PR#164](https://github.com/Lkcoinse/Lkcoinse/pull/164)
...
Satoshi Nakamoto implemented a 32MB message limit in Lkcoinse. To prevent any
...
Lkcoinse Unlimited differentiates useful and less useful data between nodes.
...
spy nodes, From version 1.0.0, Lkcoinse Unlimited Nodes track if peers supply
useful data.
...
Further information:
- [GitHub #PR 62](https://github.com/Lkcoinse/Lkcoinse/pull/62)

Request Manager extensions
--------------------------

Lkcoinse Unlimited uses a Request Manager that tracks source nodes for blocks and
...
Further information:
- [GitHub PR #79](https://github.com/Lkcoinse/Lkcoinse/pull/79)
- [GitHub PR #229](https://github.com/Lkcoinse/Lkcoinse/pull/229)

Xthin block propagation optimizations
-------------------------------------

Based on many months of live traffic observation, Lkcoinse Unlimited implemented
...
Further information:
- [GitHub PR #131](https://github.com/Lkcoinse/Lkcoinse/pull/131)
- [GitHub PR #173](https://github.com/Lkcoinse/Lkcoinse/pull/173)
- [GitHub PR #174](https://github.com/Lkcoinse/Lkcoinse/pull/174)
- [GitHub PR #176](https://github.com/Lkcoinse/Lkcoinse/pull/176)
- [GitHub PR #191](https://github.com/Lkcoinse/Lkcoinse/pull/191)
...
Further information:
- [GitHub PR #42](https://github.com/Lkcoinse/Lkcoinse/pull/42)
- [GitHub PR #100](https://github.com/Lkcoinse/Lkcoinse/pull/100)
...
lkcoinse.conf, Lkcoinse Unlimited introduces a new format using the dot notation.
For example, to manage connections users can now specify "net.maxConnections" or
"net.maxOutboundConnections".

Parameter access is unified -- the same name is used in lkcoinse.conf, via
command line arguments, or via lkcoinse-cli (et al). Old parameters in the legacy
format are still supported, while new parameters, like the excessive block size,
are only implemented in the new format.
Try `lkcoinse-cli get help "\*"` to see the list of parameters.
...
luke-jr, sipa, 21 and others. Lkcoinse Unlimited aims to create its own DNS seed
capable of special requirements such as supporting the service bit of XTHIN. By
adopting and adjusting Sipa's code for a DNS seed Lkcoinse Unlimited fixed a
minor bug.

The DNS Seed of Lkcoinse Unlimited is currently activated on the NOL (No Limit)
Net, the testnet for Lkcoinse Unlimited's Emergent Consensus. It is expected to
go live on the mainnet soon.

Further information:
- [Github sipa/bicoin-seeder PR# 42](https://github.com/sipa/lkcoinse-seeder/pull/42)
...
During tests it was discovered that the shutdown of lkcoinsed can cause problems.
...
reorganizing global variables in a single file Lkcoinse Unlimited fixed the
destruction order and eliminates this bug. This is especially helpful for
running lkcoinsed test suite.

Further information:
- [GitHub PR #67](https://github.com/Lkcoinse/Lkcoinse/pull/67)

Lock order checking in debug mode
---------------------------------

Lkcoinse's debug mode checks that locks are taken in a consistent order to
...
offered. This was backported from the Lkcoinse Core project and adjusted for the
Lkcoinse Unlimited environment.


Imported Commits
================

Changes from the open source Lkcoinse Core Project
-------------------------------------------------
...
Lkcoinse Unlimited has therefore cherry-picked a number of Lkcoinse Core 0.13.x
updates and upgrades which align with BU's onchain-scaling and wider goals to
advance Lkcoinse. We would like to thank developers Cory Fields, Jonas
...
- `37214f7` Rebrand Lkcoinse Core as Lkcoinse
...
- `e9bba70` [qa] print out seed in LkcoinseTestFramework instead of requiring tests to do it
...
- `4465a7f` Merge branch 'accurate_sighash_tracking' of https://github.com/dgenr8/lkcoinse into sigop_sighash
...
- `0e3f81b` Fix static lkcoinse-qt build for x86_64-linux-gnu arch.
...
- `61930df` Fix lkcoinse_qt.m4 and fix-xcb-include-order.patch
