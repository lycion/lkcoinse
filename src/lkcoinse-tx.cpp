// Copyright (c) 2009-2015 The Lkcoinse developers
...
// BU add lockstack stuff here for lkcoinse-cli, because I need to carefully
// order it in globals.cpp for lkcoinsed and lkcoinse-qt
...
static int AppInitRawTx(int argc, char *argv[])
{
    ...
    // Parameters
    //
    AllowedArgs::LkcoinseTx allowedArgs;
    try
    {
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help") || mapArgs.count("-version"))
    {
        ...
        std::string strUsage =
            strprintf(_("%s lkcoinse-tx utility version"), _(PACKAGE_NAME)) + " " + FormatFullVersion() + "\n";
        ...
        strUsage = "\n" + _("Usage:") + "\n" + "  lkcoinse-tx [options] <hex-tx> [commands]  " +
                   _("Update hex-encoded lkcoinse transaction") + "\n" + "  lkcoinse-tx [options] -create [commands]   " +
                   _("Create hex-encoded lkcoinse transaction") + "\n" + "\n";
...
static int CommandLineRawTx(int argc, char *argv[])
{
    ...
    try
    {
        ...
        if (!fCreateBlank)
        {
            ...
            // param: hex-encoded lkcoinse transaction
