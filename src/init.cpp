...
#endif
    GenerateLkcoinses(false, 0, Params());
    StopNode();
    StopTorControl();
...
/** Sanity checks
 *  Ensure that Lkcoinse is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // UAHF - LkcoinseCash service bit
    nLocalServices |= NODE_LKCOINSE_CASH;
    ...
    // Make sure only a single Lkcoinse process is using the data directory.
    ...
    LOGA("Using config file %s\n", GetConfigFile(GetArg("-conf", LKCOINSE_CONF_FILENAME)).string());
...
// ********************************************************* Step 7: load wallet
    ...
    // It is possible that we need to override the old settings in QT and lkcoinse.conf.
