    ...
    size_t *nHashedOut)
{
    if (nIn >= txTo.vin.size())
    {
        ...
        // See also: https://lists.linuxfoundation.org/pipermail/lkcoinse-dev/2014-November/006878.html
...
static uint256 SignatureHashLkcoinseCash(const CScript &scriptCode,
    const CTransaction &txTo,
    unsigned int nIn,
    uint32_t nHashType,
    const CAmount &amount,
    ...
    size_t *nHashedOut)
{
    ...
    {
        return SignatureHashLkcoinseCash(scriptCode, txTo, nIn, nHashType, amount, nHashedOut);
    }
