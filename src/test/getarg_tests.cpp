// Copyright (c) 2012-2015 The Lkcoinse Core developers
...
static void ResetArgs(const std::string &strArg, Kind kind = LKCOINSED)
{
    ...
    vecArg.insert(vecArg.begin(), "testlkcoinse");
    ...
    else if (kind == LKCOINSED)
        ParseParameters(vecChar.size(), &vecChar[0], AllowedArgs::Lkcoinsed(&tweaks));
    else
        ParseParameters(vecChar.size(), &vecChar[0], AllowedArgs::LkcoinseCli());
}
...
BOOST_AUTO_TEST_CASE(tweakArgs)
{
    ...
    // Test lkcoinse-cli accepts unknown tweaks
    ResetArgs("-some.tweak=something", LKCOINSE_CLI);
