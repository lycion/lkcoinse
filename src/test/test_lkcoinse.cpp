...
#define BOOST_TEST_MODULE Lkcoinse Test Suite

#include "test_lkcoinse.h"
...
TestingSetup::TestingSetup(const std::string &chainName) : BasicTestingSetup(chainName)
{
    ...
    ClearDatadirCache();
    pathTemp = GetTempPath() / strprintf("test_lkcoinse_%lu_%i", (unsigned long)GetTime(), (int)(GetRand(1 << 30)));
...
struct StartupShutdown
{
    StartupShutdown()
    {
        ...
        optDef.add_options()("testhelp", "program options information")(
            ...
            "log_lkcoinse", value<std::string>()->required(), "lkcoinse logging destination (console, none)");
        variables_map opts;
        ...
        if (opts.count("log_lkcoinse"))
        {
            std::string s = opts["log_lkcoinse"].as<std::string>();
            if (s == "console")
            {
                /* To enable this, add
                   -- --log_lkcoinse console
                   to the end of the test_lkcoinse argument list. */
