...
{
    return QCoreApplication::translate("lkcoinse-unlimited", psz).toStdString();
}
    ...
    QTranslator &translator)
{
    ...
    // Load e.g. lkcoinse_de.qm (shortcut "de" needs to be defined in lkcoinse.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. lkcoinse_de_DE.qm (shortcut "de_DE" needs to be defined in lkcoinse.qrc)
...
/** Class encapsulating Lkcoinse startup and shutdown.
 * Allows running startup and shutdown in a different thread from the UI thread.
 */
class LkcoinseCore : public QObject
{
    Q_OBJECT
public:
    explicit LkcoinseCore();
...
/** Main Lkcoinse application object */
class LkcoinseApplication : public QApplication
{
    Q_OBJECT
public:
    explicit LkcoinseApplication(int &argc, char **argv);
    ~LkcoinseApplication();
    ...
    /// Get window identifier of QMainWindow (LkcoinseGUI)
...
#include "lkcoinse.moc"

LkcoinseCore::LkcoinseCore() : QObject() {}
void LkcoinseCore::handleRunawayException(const std::exception *e)
{
    PrintExceptionContinue(e, "Runaway exception");
    Q_EMIT runawayException(QString::fromStdString(strMiscWarning));
}

void LkcoinseCore::initialize(Config *cfg)
{
    Config &config(*cfg);
    try
    {
...
LkcoinseApplication::LkcoinseApplication(int &argc, char **argv)
...
LkcoinseApplication::~LkcoinseApplication()
{
    if (coreThread)
    {
...
#ifdef ENABLE_WALLET
void LkcoinseApplication::createPaymentServer() { paymentServer = new PaymentServer(this); }
#endif

void LkcoinseApplication::createPlatformStyle()
{
...
void LkcoinseApplication::createOptionsModel(bool resetSettings)
{
    optionsModel = new OptionsModel(NULL, resetSettings);
    unlimitedModel = new UnlimitedModel(); // BU
}

void LkcoinseApplication::createWindow(const Config *config, const NetworkStyle *networkStyle)
{
    window = new LkcoinseGUI(config, platformStyle, networkStyle, 0);
...
void LkcoinseApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
...
void LkcoinseApplication::startThread()
{
    if (coreThread)
        return;
    coreThread = new QThread(this);
    LkcoinseCore *executor = new LkcoinseCore();
...
void LkcoinseApplication::parameterSetup()
{
    InitLogging();
    InitParameterInteraction();
}

void LkcoinseApplication::requestInitialize(Config &config)
{
...
void LkcoinseApplication::requestShutdown()
{
...
void LkcoinseApplication::initializeResult(int retval)
{
    ...
    if (retval)
    {
        ...
        if (pwalletMain)
        {
            ...
            window->addWallet(LkcoinseGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(LkcoinseGUI::DEFAULT_WALLET);
        ...
        // lkcoinsecash: URIs or payment requests:
...
void LkcoinseApplication::shutdownResult(int retval)
{
    qDebug() << __func__ << ": Shutdown result: " << retval;
    quit(); // Exit main loop after shutdown finished
}

void LkcoinseApplication::handleRunawayException(const QString &message)
{
    QMessageBox::critical(0, "Runaway exception",
        LkcoinseGUI::tr("A fatal error occurred. Lkcoinse can no longer continue safely and will quit.") +
            QString("\n\n") + message);
    ::exit(EXIT_FAILURE);
}

WId LkcoinseApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
int main(int argc, char *argv[])
{
    ...
    Q_INIT_RESOURCE(lkcoinse);
    Q_INIT_RESOURCE(lkcoinse_locale);

    LkcoinseApplication app(argc, argv);
    ...
    AllowedArgs::LkcoinseQt allowedArgs(&tweaks);
    try
    {
    ...
    catch (const std::exception &e)
    {
        QMessageBox::critical(
            0, QObject::tr("Lkcoinse"), QObject::tr("Error: Cannot parse program options: %1.").arg(e.what()));
        return EXIT_FAILURE;
    }
    ...
    // For BUCash, first try to migrate from LCS BU settings
    ...
    // This is necessary in case the user wants to run side-by-side LCS chain and BCH chain nodes
    ...
    /// 7. Determine availability of data directory and parse lkcoinse.conf
    ...
    // lkcoinsecash: links repeatedly have their payment requests routed to this
    // process:
