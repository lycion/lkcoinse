...
#include "lkcoinseaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Lkcoinse address (e.g. %1)")
                                   .arg(QString::fromStdString(DummyAddress(params, GetConfig()))));
    widget->setValidator(new LkcoinseAddressEntryValidator(params.CashAddrPrefix(), parent));
    widget->setCheckValidator(new LkcoinseAddressCheckValidator(parent));
}
...
QString lkcoinseURIScheme(const CChainParams &params, bool useCashAddr)
{
    if (!useCashAddr)
    {
        return "lkcoinsecash";
...
QString lkcoinseURIScheme(const Config &cfg)
{
    return lkcoinseURIScheme(cfg.GetChainParams(), cfg.UseCashAddrEncoding());
}
...
bool parseLkcoinseURI(const QString &scheme, const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if (!i->second.isEmpty())
            {
                if (!LkcoinseUnits::parse(LkcoinseUnits::BCH, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parseLkcoinseURI(const QString &scheme, QString uri, SendCoinsRecipient *out)
{
    //
    //    Cannot handle this later, because lkcoinsecash://
    ...
    return parseLkcoinseURI(scheme, uriInstance, out);
}

QString formatLkcoinseURI(const Config &cfg, const SendCoinsRecipient &info)
{
    QString ret = info.address;
    if (!cfg.UseCashAddrEncoding())
    {
        // prefix address with uri scheme for base58 encoded addresses.
        ret = (lkcoinseURIScheme(cfg) + ":%1").arg(ret);
    }
    int paramCount = 0;
    ...
    if (info.amount)
    {
        ret += QString("?amount=%1")
                   .arg(LkcoinseUnits::format(LkcoinseUnits::BCH, info.amount, false, LkcoinseUnits::separatorNever));
        paramCount++;
    }
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Lkcoinse.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Lkcoinse (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Lkcoinse (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for Lkcoinse*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "lkcoinse.desktop";
    return GetAutostartDir() / strprintf("lkcoinse-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a lkcoinse.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Lkcoinse\n";
        else
            optionFile << strprintf("Name=Lkcoinse (%s)\n", chain);
...
bool GetStartOnSystemStartup()
{
    CFURLRef lkcoinseAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    LSSharedFileListRef loginItems = LSSharedFileListCreate(NULL, kLSSharedFileListSessionLoginItems, NULL);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, lkcoinseAppUrl);
    // findStartupItemInList retains the item it returned, need to release
    if (foundItem)
        CFRelease(foundItem);
    CFRelease(loginItems);
    CFRelease(lkcoinseAppUrl);
    return !!foundItem;
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    {
        // add lkcoinse app to startup item list
        LSSharedFileListInsertItemURL(
            loginItems, kLSSharedFileListItemBeforeFirst, NULL, NULL, lkcoinseAppUrl, NULL, NULL);
    }
...
QString uriPrefix() { return "lkcoinsecash"; }
} // namespace GUIUtil
