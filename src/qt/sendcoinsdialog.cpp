...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    processSendCoinsReturn(prepareStatus, LkcoinseUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
    ...
    Q_FOREACH (const SendCoinsRecipient &rcp, currentTransaction.getRecipients())
    {
        ...
        else
        {
            ...
            QString amount =
                "<b>" + LkcoinseUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), rcp.amount);
            amount.append("</b>");
    ...
    if (txFee > 0)
    {
        ...
        questionString.append(LkcoinseUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    Q_FOREACH (LkcoinseUnits::Unit u, LkcoinseUnits::availableUnits())
    {
        if (u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(LkcoinseUnits::formatHtmlWithUnit(u, totalAmount));
    }
    questionString.append(
        tr("Total Amount %1")
            .arg(LkcoinseUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount)));
    ...
    const CAmount &watchImmatureBalance)
{
    ...
    if (model && model->getOptionsModel())
    {
        ui->labelBalance->setText(LkcoinseUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), balance));
    }
}
    ...
    case WalletModel::AbsurdFee:
        msgParams.first =
            tr("A fee higher than %1 is considered an absurdly high fee.")
                .arg(LkcoinseUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), maxTxFee.Value()));
        break;
...
void SendCoinsDialog::updateFeeMinimizedLabel()
{
    ...
    else
    {
        ui->labelFeeMinimized->setText(
            LkcoinseUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), ui->customFee->value()) +
    ...
    if (model && model->getOptionsModel())
        ui->checkBoxMinimumFee->setText(
            tr("Pay only the required fee of %1")
                .arg(LkcoinseUnits::formatWithUnit(
...
void SendCoinsDialog::updateSmartFeeLabel()
{
    ...
    {
        ui->labelSmartFee->setText(LkcoinseUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(),
...
void SendCoinsDialog::coinControlChangeEdited(const QString &text)
{
    if (model && model->getAddressTableModel())
    {
        ...
        {
            // Invalid address
            ui->labelCoinControlChangeLabel->setText(tr("Warning: Invalid Lkcoinse address"));
        }
        else
        {
            // Valid address
