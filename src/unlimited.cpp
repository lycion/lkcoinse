...
void UnlimitedSetup(void)
{
    ...
    GenerateLkcoinses(GetBoolArg("-gen", DEFAULT_GENERATE), GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), Params());
}
...
{
    return FormatParagraph(strprintf(_("Copyright (C) 2015-%i The Lkcoinse Unlimited Developers"), COPYRIGHT_YEAR)) +
           "\n\n" +
           FormatParagraph(strprintf(_("Portions Copyright (C) 2009-%i The Lkcoinse Core Developers"), COPYRIGHT_YEAR)) +
           "\n\n" +
           FormatParagraph(strprintf(_("Portions Copyright (C) 2014-%i The Lkcoinse XT Developers"), COPYRIGHT_YEAR)) +
...
static bool ProcessBlockFound(const CBlock *pblock, const CChainParams &chainparams)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("LkcoinseMiner: generated block is stale");
    }
...

    {
        ...
        if (!ProcessNewBlock(state, chainparams, nullptr, pblock, true, nullptr, false))
            return error("LkcoinseMiner: ProcessNewBlock, block not accepted");
    }
...
void static LkcoinseMiner(const CChainParams &chainparams)
{
    LOGA("LkcoinseMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("lkcoinse-miner");
    ...
    try
    {
        ...
        while (true)
        {
            ...
            {
                LOGA("Error in LkcoinseMiner: Keypool ran out, please call keypoolrefill before restarting the "
                     "mining thread\n");
                return;
            }
            ...
            LOGA("Running LkcoinseMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
                pblock->GetBlockSize());
    ...
    {
        LOGA("LkcoinseMiner terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LOGA("LkcoinseMiner runtime error: %s\n", e.what());
        return;
    }
}

void GenerateLkcoinses(bool fGenerate, int nThreads, const CChainParams &chainparams)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&LkcoinseMiner, boost::cref(chainparams)));
}
...
UniValue getgenerate(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error("getgenerate\n"
                            ...
                            std::string(LKCOINSE_CONF_FILENAME) +
                            " setting gen)\n"
...
UniValue setgenerate(const UniValue &params, bool fHelp)
{
    ...
    GenerateLkcoinses(fGenerate, nGenProcLimit, Params());
...
extern UniValue getminercomment(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error("getminercomment\n"
                            ...
                            "after the Lkcoinse Unlimited parameters."
                            "\nResult\n"
...
extern UniValue setminercomment(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error("setminercomment\n"
                            ...
                            "the Lkcoinse Unlimited parameters.\n Comments that are too long will be truncated."
                            "\nExamples:\n" +
                            HelpExampleCli("setminercomment", "\"lkcoinse is fundamentally emergent consensus\"") +
                            HelpExampleRpc("setminercomment", "\"lkcoinse is fundamentally emergent consensus\""));
...
UniValue validateblocktemplate(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error(
            ...
            "See https://en.lkcoinse.it/wiki/BIP_0022 for full specification.\n"
...
UniValue getaddressforms(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 1)
        throw runtime_error("getaddressforms \"address\"\n"
                            ...
                            "\"lkcoinsecash\": \"lkcoinsecash prefixed address\",\n"
    ...
    node.push_back(Pair("lkcoinsecash", cashAddr));
