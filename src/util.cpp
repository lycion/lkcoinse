...
const char *const LKCOINSE_CONF_FILENAME = "lkcoinse.conf";
const char *const LKCOINSE_PID_FILENAME = "lkcoinsed.pid";
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\Lkcoinse
// Windows >= Vista: C:\Users\Username\AppData\Roaming\Lkcoinse
// Mac: ~/Library/Application Support/Lkcoinse
// Unix: ~/.lkcoinse
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Lkcoinse";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Lkcoinse";
#else
    // Unix
    return pathRet / ".lkcoinse";
#endif
#endif
}
...
{
    fs::ifstream streamConfig(GetConfigFile(GetArg("-conf", LKCOINSE_CONF_FILENAME)));
    if (!streamConfig.good())
        return; // No lkcoinse.conf file is OK
    ...
    {
        // Don't overwrite existing settings so command line settings override lkcoinse.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(GetArg("-pid", LKCOINSE_PID_FILENAME));
