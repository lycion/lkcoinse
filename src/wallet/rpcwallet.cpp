...
UniValue getnewaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error("getnewaddress ( \"account\" )\n"
                            "\nReturns a new Lkcoinse address for receiving payments.\n"
                            ...
                            "\nResult:\n"
                            "\"lkcoinseaddress\"    (string) The new lkcoinse address\n"
                            "\nExamples:\n" +
...
UniValue getaccountaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "\nDEPRECATED. Returns the current Lkcoinse address for receiving payments to this account.\n"
            "\nArguments:\n"
            ...
            "\nResult:\n"
            "\"lkcoinseaddress\"   (string) The account lkcoinse address\n"
            "\nExamples:\n" +
...
UniValue getrawchangeaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 1)
        throw runtime_error("getrawchangeaddress\n"
                            "\nReturns a new Lkcoinse address, for receiving change.\n"
...
UniValue setaccount(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount \"lkcoinseaddress\" \"account\"\n"
            "\nDEPRECATED. Sets the account associated with the given address.\n"
            "\nArguments:\n"
            "1. \"lkcoinseaddress\"  (string, required) The lkcoinse address to be associated with an account.\n"
    ...
    {
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Lkcoinse address");
    }
...
UniValue getaccount(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error("getaccount \"lkcoinseaddress\"\n"
                            "\nDEPRECATED. Returns the account associated with the given address.\n"
                            "\nArguments:\n"
                            "1. \"lkcoinseaddress\"  (string, required) The lkcoinse address for account lookup.\n"
                            "\nResult:\n"
...
UniValue getaddressesbyaccount(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error("getaddressesbyaccount \"account\"\n"
                            ...
                            "  \"lkcoinseaddress\"  (string) a lkcoinse address associated with the given account\n"
                            "  ,...\n"
                            "]\n"
                            "\nExamples:\n" +
...
UniValue sendtoaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            "sendtoaddress \"lkcoinseaddress\" amount ( \"comment\" \"comment-to\" subtractfeefromamount )\n"
            "\nSend an amount to a given address.\n" +
            HelpRequiringPassphrase() + "\nArguments:\n"
                                        "1. \"lkcoinseaddress\"  (string, required) The lkcoinse address to send to.\n"
            ...
            "being sent.\n"
            "                             The recipient will receive less lkcoinses than you enter in the amount "
            "field.\n"
            "\nResult:\n"
...
UniValue listaddressgroupings(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp)
        throw runtime_error("listaddressgroupings\n"
                            ...
                            "  [\n"
                            "    [\n"
                            "      \"lkcoinseaddress\",     (string) The lkcoinse address\n"
...
UniValue signmessage(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage \"lkcoinseaddress\" \"message\"\n"
            "\nSign a message with the private key of an address" +
            HelpRequiringPassphrase() +
            "\n"
            "\nArguments:\n"
            "1. \"lkcoinseaddress\"  (string, required) The lkcoinse address to use for the private key.\n"
...
UniValue getreceivedbyaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error("getreceivedbyaddress \"lkcoinseaddress\" ( minconf )\n"
                            "\nReturns the total amount received by the given lkcoinseaddress in transactions with at "
                            "least minconf confirmations.\n"
                            "\nArguments:\n"
                            "1. \"lkcoinseaddress\"  (string, required) The lkcoinse address for transactions.\n"
...
UniValue sendfrom(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 3 || params.size() > 6)
        throw runtime_error(
            "sendfrom \"fromaccount\" \"tolkcoinseaddress\" amount ( minconf \"comment\" \"comment-to\" )\n"
            "\nDEPRECATED (use sendtoaddress). Sent an amount from an account to a lkcoinse address." +
            HelpRequiringPassphrase() +
            "\n"
            "\nArguments:\n"
            "1. \"fromaccount\"       (string, required) The name of the account to send funds from. May be the "
            "default account using \"\".\n"
            "2. \"tolkcoinseaddress\"  (string, required) The lkcoinse address to send funds to.\n"
...
UniValue sendmany(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 5)
        throw runtime_error(
            ...
            "    {\n"
            "      \"address\":amount   (numeric or string) The lkcoinse address is the key, the numeric amount (can be "
            "string) in " +
            CURRENCY_UNIT +
            " is the value\n"
            "      ,...\n"
            ...
            "                           Those recipients will receive less lkcoinses than you enter in their "
    ...
    for (const std::string &name_ : keys)
    {
        ...
        if (!IsValidDestination(dest))
        {
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Lkcoinse address: ") + name_);
        }
...
UniValue addmultisigaddress(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 2 || params.size() > 3)
    {
        string msg =
            ...
            "Each key is a Lkcoinse address or hex-encoded public key.\n"
            "If 'account' is specified (DEPRECATED), assign address to that account.\n"

            "\nArguments:\n"
            "1. nrequired        (numeric, required) The number of required signatures out of the n keys or "
            "addresses.\n"
            "2. \"keysobject\"   (string, required) A json array of lkcoinse addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"address\"  (string) lkcoinse address or hex-encoded public key\n"
            "       ...,\n"
            "     ]\n"
            "3. \"account\"      (string, optional) DEPRECATED. An account to assign the addresses to.\n"

            "\nResult:\n"
            "\"lkcoinseaddress\"  (string) A lkcoinse address associated with the keys.\n"
...
UniValue listtransactions(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 4)
        throw runtime_error(
            ...
            "    \"address\":\"lkcoinseaddress\",    (string) The lkcoinse address of the transaction. Not present for \n"
...
UniValue gettransaction(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "      \"address\" : \"lkcoinseaddress\",   (string) The lkcoinse address involved in the transaction\n"
...
UniValue walletpassphrase(const UniValue &params, bool fHelp)
{
    ...
    if (pwalletMain->IsCrypted() && (fHelp || params.size() != 2))
        throw runtime_error(
            ...
            "This is needed prior to performing transactions related to private keys such as sending lkcoinses\n"
            "\nArguments:\n"
...
UniValue encryptwallet(const UniValue &params, bool fHelp)
{
    ...
    if (!pwalletMain->IsCrypted() && (fHelp || params.size() != 1))
        throw runtime_error(
            ...
            "\nNow set the passphrase to use the wallet, such as for signing or sending lkcoinse\n" +
            HelpExampleCli("walletpassphrase", "\"my pass phrase\"") + "\nNow we can so something like sign\n" +
            HelpExampleCli("signmessage", "\"lkcoinseaddress\" \"test message\"") +
    ...
    StartShutdown();
    return "wallet encrypted; Lkcoinse server stopping, restart to run with encrypted wallet. The keypool has been "
...
UniValue lockunspent(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "A locked transaction output will not be chosen by automatic coin selection, when spending lkcoinses.\n"
...
UniValue listunspent(const UniValue &params, bool fHelp)
{
    ...
    if (fHelp || params.size() > 3)
        throw runtime_error(
            ...
            "3. \"addresses\"    (string) A json array of lkcoinse addresses to filter\n"
            "    [\n"
            "      \"address\"   (string) lkcoinse address\n"
            "      ,...\n"
            "    ]\n"
            "\nResult\n"
            ...
            "    \"vout\" : n,               (numeric) the vout value\n"
            "    \"address\" : \"address\",  (string) the lkcoinse address\n"
    ...
    if (params.size() > 2)
    {
        ...
        for (unsigned int idx = 0; idx < inputs.size(); idx++)
        {
            ...
            if (!IsValidDestination(address))
                throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid Lkcoinse address: ") + input.get_str());
