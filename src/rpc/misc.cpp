...
UniValue getinfo(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"balance\": xxxxxxx,         (numeric) the total lkcoinse balance of the wallet\n"
            ...
            CURRENCY_UNIT +
            "/kB\n"
            "  \"fork\": \"...\"             (string) \"Lkcoinse Cash\" or \"Lkcoinse\".  Will display as Lkcoinse "
            "pre-fork.\n"
    ...
    obj.push_back(Pair("fork", "Lkcoinse Cash"));
...
UniValue validateaddress(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"lkcoinseaddress\"\n"
            "\nReturn information about the given lkcoinse address.\n"
            "\nArguments:\n"
            "1. \"lkcoinseaddress\"     (string, required) The lkcoinse address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,       (boolean) If the address is valid or not. If not, this is the only "
            "property returned.\n"
            "  \"address\" : \"lkcoinseaddress\", (string) The lkcoinse address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: Lkcoinse address and we have full public key:
...
UniValue createmultisig(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg =
            ...
            "2. \"keys\"       (string, required) A json array of keys which are lkcoinse addresses or hex-encoded "
            "public keys\n"
            "     [\n"
            "       \"key\"    (string) lkcoinse address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
UniValue verifymessage(const UniValue &params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"lkcoinseaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"lkcoinseaddress\"  (string, required) The lkcoinse address to use for the signature.\n"
