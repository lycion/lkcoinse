...
// Copyright (c) 2017-2018 The Lkcoinse Unlimited developers
...
// These globals are needed here so lkcoinse-cli can link
const std::string CURRENCY_UNIT = "BCH";
const std::string DEFAULT_TOR_CONTROL = "127.0.0.1:9051";
const char DEFAULT_RPCCONNECT[] = "127.0.0.1";

// Variables for traffic shaping.  Needed here so lkcoinse-cli can link
...
static void addConfigurationLocationOptions(AllowedArgs &allowedArgs)
{
    allowedArgs.addHeader(_("Configuration location options:"))
        .addArg(
            "conf=<file>", requiredStr, strprintf(_("Specify configuration file (default: %s)"), LKCOINSE_CONF_FILENAME))
        .addArg(
...
#ifndef WIN32
        .addArg("pid=<file>", requiredStr, strprintf(_("Specify pid file (default: %s)"), LKCOINSE_PID_FILENAME))
#endif
...
static void addWalletOptions(AllowedArgs &allowedArgs)
{
    ...
    allowedArgs.addHeader(_("Wallet options:"))
        ...
        .addArg("usecashaddr", optionalBool,
            _("Use Lkcoinse Cash Address for destination encoding (Activates by default Jan 14, 2017)"));
#endif
}
...
static void addRpcServerOptions(AllowedArgs &allowedArgs)
{
    allowedArgs.addHeader(_("RPC server options:"))
        ...
        // Although a node does not use rpcconnect it must be allowed because LkcoinseCli also uses the same config file
...
// lkcoinse-cli does not know about tweaks so we have to silently ignore unknown options
LkcoinseCli::LkcoinseCli() : AllowedArgs(true)
{
    addHelpOptions(*this);
...
Lkcoinsed::Lkcoinsed(CTweakMap *pTweaks) : AllowedArgs(false) { addAllNodeOptions(*this, HMM_LKCOINSED, pTweaks); }
LkcoinseQt::LkcoinseQt(CTweakMap *pTweaks) : AllowedArgs(false) { addAllNodeOptions(*this, HMM_LKCOINSE_QT, pTweaks); }
LkcoinseTx::LkcoinseTx() : AllowedArgs(false)
{
    addHelpOptions(*this);
    addChainSelectionOptions(*this);
    ...
    addHeader(_("Transaction options:"))
        ...
        .addDebugArg("", optionalBool, "Read hex-encoded lkcoinse transaction from stdin.");
}

ConfigFile::ConfigFile(CTweakMap *pTweaks) : AllowedArgs(false)
{
    // Merges all allowed args from LkcoinseCli, Lkcoinsed, and LkcoinseQt.
    // Excludes args from LkcoinseTx, because lkcoinse-tx does not read
    // from the config file. Does not set a help message, because the
    // program does not output a config file help message anywhere.

    LkcoinseCli lkcoinseCli;
    Lkcoinsed lkcoinsed(pTweaks);
    LkcoinseQt lkcoinseQt;

    m_args.insert(lkcoinseCli.getArgs().begin(), lkcoinseCli.getArgs().end());
    m_args.insert(lkcoinsed.getArgs().begin(), lkcoinsed.getArgs().end());
    m_args.insert(lkcoinseQt.getArgs().begin(), lkcoinseQt.getArgs().end());
}
