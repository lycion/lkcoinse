...
// Copyright (c) 2009-2017 The Lkcoinse developers
 ...
 *
 //www.lkcoinse.org/),
 * This is the developer documentation of the reference client for an experimental new digital currency called Lkcoinse
 (https:
 * which enables instant payments to anyone, anywhere in the world. Lkcoinse uses peer-to-peer technology to operate
...
bool AppInit(int argc, char *argv[])
{
    ...
    // Parameters
    //
    // If Qt is used, parameters/lkcoinse.conf are parsed in qt/lkcoinse.cpp's main()
    AllowedArgs::Lkcoinsed allowedArgs(&tweaks);
    try
    {
    ...
    try
    {
        ...
        {
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "lkcoinse:") &&
                !boost::algorithm::istarts_with(argv[i], "lkcoinsecash:"))
            {
                fCommandLine = true;
                break;
            }
        ...
        if (fCommandLine)
        {
            fprintf(stderr, "Error: There is no RPC client functionality in lkcoinsed anymore. Use the lkcoinse-cli "
                            "utility instead.\n");
            return false;
        }
#ifndef WIN32
        ...
        if (fDaemon)
        {
            fprintf(stdout, "Lkcoinse server starting\n");
...
int main(int argc, char *argv[])
{
    ...
    // Connect lkcoinsed signal handlers
    noui_connect();
